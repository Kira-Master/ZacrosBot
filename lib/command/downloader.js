...
client.sendMessage(msg.from,(await client.getBuffer(type.link)).buffer,data.type+'Message',{...data,contextInfo:{externalAdReply:{title:type.judul,description:`Kira-Master`,mediaType:"VIDEO",thumbnail,mediaUrl:isUrl[0],thumbnailUrl:type.thumb}}})
...
cmd.on('instagramdl',['igdl'],['downloader'],async(msg,{query,client})=> {
if (!query) return client.sendText(msg.from,`Masukkan link instagram\nContoh : .igdl https://www.instagram.com/p/CUqy9E6Bq9D/?utm_medium=copy_link`,msg)
let anu = body.slice(6)
let etto = await functions.axios.get(`https://zacrosapi.herokuapp.com/downloader/igdl?link=${anu}`)
await client.sendText(msg.from,'Tunggu sebentar ya..',msg)
return client.sendMessage(msg.from,(await client.getBuffer(etto.link)).buffer,'videoMessage',{caption:`Nih`,quoted:msg})
},{param:functions.needed('link instagram')});
